import streamlit as st
import random

st.set_page_config(page_title="나와 어울리는 책은?", page_icon="📚", layout="centered")

st.title("📚 나와 어울리는 책은?")
st.write(
    "간단한 7문항 심리테스트로 당신의 독서 성향을 분석하고, "
    "당신에게 어울리는 책 3권을 추천해드릴게요!"
)

# ----------------------------
# 질문 데이터
# ----------------------------
questions = [
    "1) 새로운 주제를 배울 때 내가 가장 흥미를 느끼는 방식은?",
    "2) 시간이 생겼을 때 내가 가장 자주 선택하는 활동은?",
    "3) 친구가 “요즘 좀 힘들다”고 말하면 나는 보통…",
    "4) 내가 책을 읽는 가장 큰 목적은?",
    "5) 다음 중 가장 끌리는 콘텐츠는?",
    "6) 어떤 책이 “좋은 책”이라고 느껴지는가?",
    "7) 내가 가장 궁금해하는 질문은 어떤 유형인가?",
]

question_choices = [
    [
        "A. 실생활에 적용할 수 있는 방법을 찾는다",
        "B. 그 주제가 삶에 어떤 의미가 있는지 생각한다",
        "C. 원리나 구조를 분석하며 이해한다",
        "D. 사회나 시대적 배경 속에서 바라본다",
        "E. 이야기나 사례를 통해 자연스럽게 몰입한다",
    ],
    [
        "A. 목표를 세우거나 자기관리 루틴을 만든다",
        "B. 깊이 있는 질문을 던지는 글을 읽는다",
        "C. 새로운 기술이나 최신 정보를 찾아본다",
        "D. 사회 이슈나 역사적 사건을 탐구한다",
        "E. 재미있는 스토리 콘텐츠를 즐긴다",
    ],
    [
        "A. 현실적인 해결책과 조언을 정리해준다",
        "B. 감정과 상황의 의미를 함께 고민한다",
        "C. 문제의 원인을 논리적으로 분석한다",
        "D. 비슷한 사회적 사례나 배경을 떠올린다",
        "E. 공감하며 이야기를 들어주는 편이다",
    ],
    [
        "A. 성장하거나 더 나은 사람이 되기 위해",
        "B. 인간과 삶을 깊이 이해하기 위해",
        "C. 새로운 지식과 정보를 얻기 위해",
        "D. 세상과 사회 구조를 이해하기 위해",
        "E. 다른 세계를 경험하고 몰입하기 위해",
    ],
    [
        "A. 성공 습관, 생산성, 동기부여 콘텐츠",
        "B. 철학적 질문이나 인문학적 에세이",
        "C. 과학·기술·미래를 다루는 영상이나 글",
        "D. 사회 문제나 역사적 흐름을 다룬 다큐",
        "E. 감정선이 강한 드라마나 소설 이야기",
    ],
    [
        "A. 읽고 나서 행동이 바뀌는 책",
        "B. 사고의 폭이 넓어지는 책",
        "C. 새로운 사실을 배우게 되는 책",
        "D. 세상을 바라보는 시야가 넓어지는 책",
        "E. 재미있고 몰입감이 뛰어난 책",
    ],
    [
        "A. “어떻게 하면 더 나은 삶을 살 수 있을까?”",
        "B. “인간은 왜 이런 선택을 할까?”",
        "C. “미래에는 어떤 기술이 세상을 바꿀까?”",
        "D. “사회는 왜 이렇게 변화해왔을까?”",
        "E. “만약 다른 삶을 산다면 어떤 이야기가 펼쳐질까?”",
    ],
]

# ----------------------------
# 장르 매핑
# ----------------------------
genre_map = {
    "A": "자기계발",
    "B": "인문/철학",
    "C": "과학/IT",
    "D": "역사/사회",
    "E": "소설",
}

# ----------------------------
# 장르별 추천 책 리스트 (임시)
# ----------------------------
book_recommendations = {
    "자기계발": ["아주 작은 습관의 힘", "그릿", "미라클 모닝", "딥 워크", "원씽"],
    "인문/철학": ["정의란 무엇인가", "죽음의 수용소에서", "철학은 어떻게 삶의 무기가 되는가", "사피엔스", "소크라테스 익스프레스"],
    "과학/IT": ["AI 2041", "코스모스", "클린 코드", "팩트풀니스", "유발 하라리의 미래기술"],
    "역사/사회": ["총, 균, 쇠", "21세기 자본", "넛지", "정치의 심리학", "역사의 쓸모"],
    "소설": ["데미안", "불편한 편의점", "1984", "달러구트 꿈 백화점", "나미야 잡화점의 기적"],
}

# ----------------------------
# session_state 초기화
# ----------------------------
if "submitted" not in st.session_state:
    st.session_state.submitted = False

for i in range(7):
    key = f"q{i+1}"
    if key not in st.session_state:
        st.session_state[key] = None


def reset_test():
    for i in range(7):
        st.session_state[f"q{i+1}"] = None
    st.session_state.submitted = False


# ----------------------------
# 점수 계산 함수
# ----------------------------
def compute_scores(answers):
    scores = {genre: 0 for genre in genre_map.values()}

    for ans in answers:
        letter = ans[0]  # "A. ..." 첫 글자
        genre = genre_map[letter]
        scores[genre] += 1

    return scores


# ----------------------------
# 질문 UI
# ----------------------------
st.divider()
st.subheader("📝 질문에 답해주세요")

for i, q in enumerate(questions):
    st.markdown(f"**{q}**")
    st.radio(
        label=f"q{i+1}",
        options=question_choices[i],
        key=f"q{i+1}",
        index=None,
        label_visibility="collapsed",
    )
    st.write("")

st.divider()

col1, col2 = st.columns(2)

with col1:
    if st.button("결과 보기", type="primary"):
        answers = [st.session_state[f"q{i+1}"] for i in range(7)]

        if any(a is None for a in answers):
            st.warning("모든 질문에 답변해 주세요!")
        else:
            st.session_state.submitted = True

with col2:
    st.button("다시 테스트하기", on_click=reset_test)

# ----------------------------
# 결과 출력
# ----------------------------
if st.session_state.submitted:
    answers = [st.session_state[f"q{i+1}"] for i in range(7)]

    scores = compute_scores(answers)

    # 가장 높은 점수 장르 찾기
    top_genre = max(scores, key=scores.get)

    st.subheader("📌 당신의 독서 성향 결과")
    st.success(f"당신은 **{top_genre}** 성향이 가장 강합니다!")

    st.write("### 📚 추천 도서 3권")

    # 랜덤 3권 추천
    추천도서 = random.sample(book_recommendations[top_genre], 3)

    for book in 추천도서:
        st.write(f"✅ {book}")

    st.info("분석 결과를 바탕으로 추천을 생성했습니다! (다음 단계: API 연동)")
